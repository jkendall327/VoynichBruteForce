...
/// </summary>
public class SingleCharEntropyRanker : IRuleAdherenceRanker
{
    ...
    public RankerResult CalculateRank(string text)
    {
        ...
        double rawDelta = Math.Abs(actualH1 - VoynichConstants.TargetH1Entropy);

        // Normalize: 0.5 bits deviation is one error unit
        double tolerance = 0.5;
        double normalizedError = Math.Pow(rawDelta / tolerance, 2);

        return new(Name, actualH1, VoynichConstants.TargetH1Entropy, normalizedError, Weight);
    }
