...
public class ZipfLawRanker(IOptions<VoynichProfile> profile) : IRuleAdherenceRanker
{
    ...
    public RankerResult CalculateRank(PrecomputedTextAnalysis analysis)
    {
        ...
        try
        {
            ...
            var slope = CalculateLogLogSlope(frequencies.AsSpan(0, count));

            var rawDelta = Math.Abs(slope - _profile.TargetZipfSlope);
    ...
    private static double CalculateLogLogSlope(ReadOnlySpan<int> sortedFrequencies)
    {
        ...
        var sampleSize = Math.Min(100, sortedFrequencies.Length);
        ...
        for (var rank = 1; rank <= sampleSize; rank++)
        {
            var frequency = sortedFrequencies[rank - 1];
            if (frequency <= 0) continue;

            var logRank = Math.Log(rank);
        ...
        var numerator = validPoints * sumLogRankLogFreq - sumLogRank * sumLogFreq;
        var denominator = validPoints * sumLogRankSquared - sumLogRank * sumLogRank;
